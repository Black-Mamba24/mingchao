...
const PromptBuilder = (() => {
  ...
  function buildAnnotationCandidateSchema(textFieldName) {
    return `[
    {
      ...
      "intro": "中文介绍，字数必须按 category 分档：person 类 180 到 260 字；place/era/office/institution 类 120 到 180 字；work/herb/weapon/event/other 类 100 到 160 字",
      "scores": {
...
- 可解释对象限定为以下类别之一：历史人物、古地名、古称、年号、职位、机构、事件、著作名称、药材、军械、直接影响当前抉择理解的生僻历史词
- 严禁为现代常用词、普通动词、形容词、代词、日常短语、情绪词、泛称、直白军政组合词、功能性文书词、抽象局势词生成解释；例如“百姓”“军队”“局势”“东西”“起来”“离开”“家人”“清军”“衙役”“清军营帐”“粮库官吏”“军心”“求援信”“粮草”“援军”“战报”“军令”“守军”“水师”“官军”“追兵”这类词一律不得进入 candidates
...
- 如果同一人物、机构、药材、著作、地名在本轮剧情前文已解释过，即使本段再次出现，也不得重复标注
- intro 用中文，字数按 category 分档：person 类 180 到 260 字；place/era/office/institution 类 120 到 180 字；work/herb/weapon/event/other 类 100 到 160 字
- 同一文本块同一词只返回一次；冗余候选数量上限为 2 个
- 以下词条在本次剧情中已经解释过，严禁再次出现在 scene、npc_dialogue、question、consequence 的 annotations 与 annotation candidates 中：${introducedTerms.length ? introducedTerms.join('、') : '（暂无）'}
- 不允许为 options 中出现的任何词生成解释
- 若同一词条已有别名、字号、简称、全称关系（如人名与字号、书名简称与全称），一旦其中任一形式已解释，其余形式也不得在后文重复标注
- 地名、机构名、书名如仅在后文增加后缀或旧制称谓（如“福州”与“福州城”“福州府”）也视为同一词条，后文不得重复标注
...
- 后续场景与选项设计必须参考玩家当前的声望、风险、历史洞察，禁止只在旁白里提到数值；必须在人物态度、局势压力、信息开放程度、可行动作类型这四项中至少改变一项`;
  ...
  "scene_annotations": [
    { "key": "专有名词唯一标识", "term": "scene中出现的词", "category": "person/place/work/herb/weapon/era/office/institution/event/other", "intro": "中文介绍，字数按 category 分档：person 类 180 到 260 字；place/era/office/institution 类 120 到 180 字；work/herb/weapon/event/other 类 100 到 160 字" }
  ],
  "scene_annotation_candidates": ${buildAnnotationCandidateSchema('scene')},
  "npc_dialogue": "当前场景中重要NPC的一句话，字数上限 50 字；如无NPC则必须为空字符串",
  "npc_annotations": [
    { "key": "专有名词唯一标识", "term": "npc_dialogue中出现的词", "category": "person/place/work/herb/weapon/era/office/institution/event/other", "intro": "中文介绍，字数按 category 分档：person 类 180 到 260 字；place/era/office/institution 类 120 到 180 字；work/herb/weapon/event/other 类 100 到 160 字" }
  ],
  ...
  "question_annotations": [
    { "key": "专有名词唯一标识", "term": "question中出现的词", "category": "person/place/work/herb/weapon/era/office/institution/event/other", "intro": "中文介绍，字数按 category 分档：person 类 180 到 260 字；place/era/office/institution 类 120 到 180 字；work/herb/weapon/event/other 类 100 到 160 字" }
  ],
...
6. JSON 格式如下：
{
  ...
  "consequence_annotations": [
    { "key": "专有名词唯一标识", "term": "consequence中出现的词", "category": "person/place/work/herb/weapon/era/office/institution/event/other", "intro": "中文介绍，字数按 category 分档：person 类 180 到 260 字；place/era/office/institution 类 120 到 180 字；work/herb/weapon/event/other 类 100 到 160 字" }
  ],
...
选择引发的后果：${ctx.consequenceText ? ctx.consequenceText.slice(0, 150) : ''}`;
    ...
    const shownTips = Array.isArray(state?.shownTips) ? state.shownTips.filter(Boolean) : [];
    const usedTipClusters = Array.isArray(state?.usedTipClusters) ? state.usedTipClusters.filter(Boolean) : [];
    const recentHistory = Array.isArray(state?.history) ? state.history.slice(-6).map(item => item?.content || '').filter(Boolean).join('\n') : '';
...
最近已经展示过的小贴士：${shownTips.length ? shownTips.join('｜') : '（暂无）'}
当前剧情已经使用过的事实簇：${usedTipClusters.length ? usedTipClusters.join('｜') : '（暂无）'}
最近场景与局势余波内容摘录：${recentHistory || '（暂无）'}

请根据上一场景刚刚发生的具体事件，生成一条与之直接相关的历史小贴士。要求：
1. 必须针对上一场景中出现的具体人物、地点、器物或事件展开，禁止泛泛而谈
2. 先在内部判断本条内容属于哪个事实簇，再输出结果；事实簇示例：人物生平、人物亲属、人物求学、政权背景、制度建置、地理形势、海战战法、城防工事、殖民据点、军政经营、著作成书、药物考证等
3. 本次输出的事实簇不得与“当前剧情已经使用过的事实簇”重复；如果同一人物或地点常见信息已讲过，必须换一个新的历史切面
4. 严禁重复、改写、近义复述最近已经展示过的小贴士，也严禁复述最近场景正文或局势余波里已经明确讲过的历史事实
5. 输出严格使用如下格式：
[FACT_CLUSTER]事实簇名称[/FACT_CLUSTER][TIP]贴士正文[/TIP]
6. 贴士正文长度必须落在 50 到 120 字区间内
7. 必须包含一个可查证的史实锚点（年号、人物、书名、地名中的至少一项）
8. 贴士正文禁止添加“小贴士：”等前缀标签，也不要输出任何额外解释`
      }
    ];
  }
