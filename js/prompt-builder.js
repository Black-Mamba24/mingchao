...
- 可解释对象包括：历史人物、地名、古地名、古称、武器名、军械名、年号、职位、机构、事件，以及现代读者不常用但会影响理解的历史词语
- 每个文本块应尽可能覆盖最值得解释的词，最多标注 5 个，不要浪费名额
- 若当前文本里出现对理解剧情起关键作用的人物姓名，优先为核心人物补足介绍；像张昺、谢贵、袁焕这类非现代常识型历史人物，通常应优先解释
- 若同一段里人物较多，为避免超出数量限制，可只保留最关键的1到3名人物介绍，配角可不解释，但至少保证当前冲突核心人物得到解释
- 优先解释“现代读者可能不熟、或虽见过但不清楚其历史语境”的词，不要解释一眼就懂的常用词
- 以下类型默认不解释，除非当前语境有非常特殊的历史含义：懿旨、监国、即位、奉天门这类现代人通常能直接理解的常见宫廷/政治词
- 优先级从高到低：关键历史人物（尤其直接影响当前场景决策者）＞古称/古地名/古今名称差异大的词（如金陵、平江、江州）＞现代不常用的历史词或器物军械旧称（如舆图、石砲）＞关键机构＞普通地名＞常见职官套语
- 若同句里同时出现“泛称”和“更具体、更少见、更有历史感的词”，优先解释后者；例如火器与石砲并存时，优先解释石砲
- 若某地名今天仍广为人知、按字面即可理解（如芜湖），且当前文本里还有更值得解释的核心人物或历史词，优先不要选它
- 如果一段里同时有人名和关键古地名，在名额允许时尽量兼顾；但人物介绍优先于一般地名
- term 必须与对应文本中的实际字面完全一致
- key 必须稳定；同一个专有名词在不同场景重复出现时，必须使用同一个 key，不要换写法
...
【人物解释补充要求】
- scene_annotations 与 npc_annotations 要优先保证当前场景核心人物有介绍，尤其是 scene 中直接点名、且会影响局势判断的人物
- 如果一句话里出现多名人物，先解释主冲突人物，再考虑地名或机构；名额不足时可以不解释配角
- 人物 intro 要直接回答“他是谁、在这段历史里扮演什么角色、为何此刻重要”
...
- 后续场景与选项设计必须参考玩家当前的声望、风险、历史洞察，不要只在旁白里提到数值，要真正改变人物态度、局势压力、信息开放程度和可行动作类型`;
  ...
  "annotation_strategy": "可选字段，简要说明本段为何选择这些词，优先覆盖古称、古地名、不常用历史词和更具体的旧称，避免解释一眼就懂的常用词",
  "options": [
