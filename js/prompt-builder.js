...
3. 只返回后果文字，不需要JSON格式`;
  ...
  function buildScenePrompt(eventData, roleId, act, scene, state) {
    ...
    if (ctx) {
      bridgeSection = `
【上一场景结尾（必须承接）】
上一场景描述摘要：${ctx.sceneText}
玩家所做的选择：${ctx.chosenOption}
选择带来的后果：${ctx.consequenceText}

【承上启下要求（严格执行）】
- 当前场景的开头必须自然地从上一场景的后果出发，时间、地点、人物的变化需要有过渡交代
- 如果地点发生跳转（如从浙江到福建、从东南到北疆），必须在场景开头用1-2句话交代时间的流逝和空间的转换，例如"数月后，你奉命北上……"
- 如果地点未变，则直接延续氛围，不得凭空另起炉灶
- 禁止场景间出现突兀的时空切换而不加任何说明
`;
    }
